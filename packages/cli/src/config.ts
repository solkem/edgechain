...
const EdgeChainPrivateStateId = 'edgechain-private-state';
...
export class StandaloneConfig implements Config {
  privateStateStoreName = EdgeChainPrivateStateId
  logDir = path.resolve(currentDir, '..', 'logs', 'standalone', `${new Date().toISOString()}.log`);
    zkConfigPath = path.resolve(currentDir, '..', '..', 'contract', 'dist', 'managed', 'edgechain');
